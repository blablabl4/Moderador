...
async def api_groups_list(username: str = Depends(get_current_username)):
    """List ALL groups the bot is part of, with is_admin flag."""
    await ensure_token()
    groups = await wpp.get_all_groups()
    all_parsed = [parse_group(g) for g in groups]
    # Return ALL groups — let the UI show admin badge
    return {"groups": all_parsed, "total": len(all_parsed)}
...
async def receive_webhook(request: Request, background_tasks: BackgroundTasks):
    ...
    if msg_type == 'sticker' and cfg.get("sticker_filter_enabled", True):
        ...
        enforce_result = await enforce_action("delete_warn", group_id, msg_id, "🚫 Stickers não são permitidos neste grupo.", sender_id=sender_id)
    ...
    if _is_status_share:
        ...
        enforce_result = await enforce_action("delete_warn", group_id, msg_id, "🚫 Compartilhamento de status não é permitido.", sender_id=sender_id)
    ...
    if group_id in phone_required:
        ...
        if is_album:
            ...
            enforce_result = await enforce_action("delete_warn", group_id, msg_id, "🚫 Álbuns (múltiplas fotos) não são permitidos. Envie apenas 1 foto por anúncio.", sender_id=sender_id)
        ...
        if not has_phone_number(text_to_check):
            ...
            enforce_result = await enforce_action("delete_warn", group_id, msg_id, "🚫 Anúncio sem número de telefone na legenda. Inclua seu contato e reposte.", sender_id=sender_id)
